...
그래프 해석 포인트:
- 요청률이 100 req/s 이상으로 상승하는 구간에서도 `Error Rate`는 0%를 유지했다.
- `Endpoint Latency Summary` 기준 `auth_login`이 가장 무거운 API였고, `party_add_members`는 threshold 안쪽에서 안정적으로 유지됐다.
- 요청률 상승 구간에서 급격한 오류 스파이크 없이 부하를 소화하는 패턴을 확인할 수 있다.
...
그래프 해석 포인트:
- 고부하 구간에서 순간 처리량이 200 req/s 이상까지 올라가도 `Error Rate`는 0%를 유지했다.
- `auth_login`은 혼합 시나리오 내에서 가장 무거운 API였지만 p95는 목표 범위 안에 머물렀다.
- `party_add_members` 역시 혼합 스트레스 조건에서 낮은 p95를 유지해 병목 재발 없이 동작했다.
...
그래프 해석 포인트:
- `scenario`와 `endpoint` 필터를 통해 `party_add_members` write-path만 분리해 관찰했다.
- 전용 스트레스 조건에서도 `Error Rate`는 0%를 유지했고, 요청률은 약 60 req/s 수준에서 안정적으로 형성됐다.
- 대시보드에 보이는 `400` 응답은 시스템 실패가 아니라, 이미 처리된 초대 대상에 대한 기대된 business response다.
...
#### 개선 전/후 비교
| 대상 | 지표 | 개선 전 | 개선 후 | 개선 효과 |
| --- | --- | ---: | ---: | --- |
| `auth_login` refresh token 저장 | 구간 latency | `2~5ms` | `0.003~0.016ms` | DB write 제거, `99% 이상` 감소 |
| `party_add_members` | endpoint p95 (`mixed`) | `54.50ms` | `24.44ms` | 약 `55.2%` 개선 |
